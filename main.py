...
def web_server():
    ...
    def get_m3u_all():
        ...
        for server in servers:
            for channel in server.channels:
                ...
                file_content += f"\n#EXTINF:-1 tvg-id=\"{channel['name']}.{server.id}.{channel['id']}\" tvg-logo=\"{channel['logo']}\" group-title=\"{channel['name']}\",{channel['name']}"
    ...
    def get_m3u(server):
        ...
        for channel in servers[int(server)].channels:
            ...
            file_content += f"\n#EXTINF:-1 tvg-id=\"{channel['name']}.{servers[int(server)].id}.{channel['id']}\" tvg-logo=\"{channel['logo']}\" group-title=\"{channel['name']}\",{channel['name']}"
