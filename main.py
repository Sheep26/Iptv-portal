...
def web_server():
    ...
    def get_m3u(server):
        ...
        for channel in servers[int(server)].channels:
            ...
            stream_url = f"{'https' if config['https'] else 'http'}://{request.url.split('/')[2]}/play/{servers[int(server)].id}/{channel['id']}?proxy={int(request.args.get('proxy', 0))}"
...
def main():
    ...
    if not os.path.exists(f"{config_dir}/config.json"):
        ...
        config = {"https": True, "iptv_servers": [], "xtream_servers": [], "channels": [], "users": []}
